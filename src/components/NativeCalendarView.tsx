        ...
        <div className="jv-calendar-sections">
          {result.events.length > 0 && (
            <section className="jv-calendar-section">
              ...
              <div className="jv-calendar-event-row jv-caption jv-muted">
              ...
              {result.events.map((event, index) => (
                <button type="button" key={event.id} className="jv-calendar-event-row jv-body jv-muted jv-calendar-item" onClick={() => setSelectedEvent(event)} title="点击查看日历详情">
                  ...
                  <span className="jv-skill-main">
                    ...
                    {event.reasons.length > 0 && <span className="jv-caption jv-muted">{event.reasons.join("；")}</span>}
                  </span>
          ...
          {result.reminders.length > 0 && (
            <section className="jv-calendar-section">
              ...
              <div className="jv-calendar-reminder-row jv-caption jv-muted">
                <span className="jv-skill-col-index">#</span>
                <span>提醒</span>
              ...
              {result.reminders.map((reminder, index) => (
                <button type="button" key={reminder.id} className="jv-calendar-reminder-row jv-body jv-muted jv-calendar-item" onClick={() => setSelectedReminder(reminder)} title="点击查看提醒详情">
                  <span className="jv-skill-col-index">{index + 1}</span>
                  <span className="jv-skill-main">
                    <span className="jv-body jv-skill-name">{reminder.title}</span>
                    {reminder.notes !== "" && <span className="jv-caption jv-muted">{reminder.notes}</span>}
                  </span>
