...
type DraggableCardId = Extract<HomeModuleId, "todo" | "summary" | "risk" | "mail">;
...
export default function DashboardView({
  ...
  const [draggingId, setDraggingId] = useState<DraggableCardId | null>(null);
  const [dropTargetId, setDropTargetId] = useState<DraggableCardId | null>(null);
  const dropCard = (toId: DraggableCardId) => {
    const from = draggingId;
    if (from === null) return;
    if (from === toId) return;
    const order = [...homeModules.order];
    const fromIndex = order.indexOf(from);
    const toIndex = order.indexOf(toId);
    order.splice(order.indexOf(from), 1);
    order.splice(fromIndex < toIndex ? toIndex : toIndex - 1, 0, from);
    onHomeModuleOrderChange(order);
  };
  const dragCardProps = (id: DraggableCardId) => ({
    className:
      "jv-home-card" +
      (draggingId === id ? " jv-home-card-dragging" : "") +
      (dropTargetId === id && draggingId !== null && draggingId !== id ? " jv-home-card-target" : ""),
    draggable: true,
  ...
  const cardModule = (id: DraggableCardId) => {
    if (id === "todo") {
      return (
        <div {...dragCardProps("todo")}>
          <Panel
